...
func TestNewButton(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, NewButton(tt.args.title, tt.args.url, tt.args.hide, tt.args.payload...))
		})
	}
}
...
func TestResponse_Buttons(t *testing.T) {
	...
	test0 := getResp(0)
	test0.Response.Buttons = []Button{b0, b1, b2}

	b3, b4 := b0, b1
	b3.Payload = "msg"
	b4.Payload = "ok"
	test1 := getResp(0)
	test1.Response.Buttons = []Button{b3, b4}
...
func TestNewPhrase(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := NewPhrase(tt.args.text, tt.args.tts)
...
func getResp(n int) *Response {
	...
	err := json.Unmarshal([]byte(source[n]), resp)
	if err != nil {
		return nil
	}
	return resp
